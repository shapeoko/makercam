package com.partkart{
	...
	public class CutPath extends Path{
		...
		/*protected override function renderCollisionArea():void{
			// we don't want to render a collision area
		}*/
			...
			for(var i:int=0; i<newseglist.length; i++){
				...
				for(var j:int=0; j<seglist.length; j++){
					if(seglist[j].p1 == newseglist[i].p2){
						...
						seglist[j] = reverseSegment(seglist[j]);
						newseglist.push(seglist[j]);
						seglist.splice(j,1);
						break;
					}
		...
		protected function reversePath():void{
			for(var i:int=0; i<seglist.length; i++){
		...
		public function offset(cutlist:Array, radius:Number, clean:Boolean = false):Array{
			...
			// remove reference to the removed cutpaths so they can be garbage collected
			//cutlist = null;
			//addChild(fillbitmap);
			//if(Global.snap == false){
				
				return processLoops();
				//
			//}
			//else{
				/*var returnpath:CutPath = new CutPath();
				returnpath.seglist = seglist;
				returnpath.clonePath();
				return new Array(returnpath);*/
		...
		public function joinSequence():void{
			//var len:int = seglist.length-1;
			for(var i:int=0; i<seglist.length-1; i++){
				if(seglist[i].p2 != seglist[i+1].p1 && Global.withinTolerance(seglist[i].p2, seglist[i+1].p1, 0.05)){
			...
			if(seglist[seglist.length-1].p2 != seglist[0].p1 && Global.withinTolerance(seglist[seglist.length-1].p2, seglist[0].p1, 0.05)){
			...
			for each(var chain in chains){
				...
				if(chain[0].p1.x > chain[chain.length-1].p2.x){
					for(i=0; i<chain.length; i++){
						...
						chain[i] = reverseSegment(chain[i]);
						chain[i].name = n;
					}
					chain.reverse();
					chain.reversed = true;
		...
		protected function getNext(current:Segment):Array{
			...
			for(var i:int=0; i<seglist.length; i++){
				...
				if(seg != current && seg.active == false){
					...
					/*else if(seg.p2 == current.p2){
						seglist[i] = reverseSegment(seg);
						nextarray.push(seglist[i]);
					}*/
				}
			}
