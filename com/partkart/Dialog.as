package com.partkart{
	...
	public class Dialog extends Sprite{
		...
		private function populateFields():void{
			...
			for(var i:int=0; i<flist.length; i++){
				...
				switch(f.type){
					...
					case "cutlist": // special element used only for exporting g-code
						...
						cutlist.width = dwidth - (hposition*2);
						cutlist.height = 200;
						cutlist.x = hposition;
			...
			submit.x = dwidth/2 - buttonwidth/2;
			submit.y = vposition;
			addChild(submit);
			...
			if(vposition + 15 > dheight){
				back.graphics.clear();
				var m:Matrix = new Matrix(dwidth/1000, 0, 0, vposition/1000,dwidth/2, vposition/2);
				back.graphics.beginGradientFill(GradientType.RADIAL, [0x444444, 0x222222],[1,1],[50,255], m);
				back.graphics.drawRoundRect(0,0,dwidth,vposition+15,20,20);
				back.graphics.endFill();
			}
		}
