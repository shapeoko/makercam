package com.partkart{
	...
	public class Path extends Sprite{
		...
		protected var dotlist:Array = new Array();
		...
		protected function overAction(e:MouseEvent):void{
			if(Global.tool == 0 && Global.dragging == false && active == false && Global.space == false){
		...
		protected function downAction(e:MouseEvent):void{
			if(Global.tool == 0 && Global.space == false){
				//e.stopPropagation();
		...
		protected function moveAction(e:MouseEvent):void{
			if(Global.tool == 0 && Global.dragging == false && active == false && Global.space == false && e.target == this){
				checkDot(this.mouseX, this.mouseY);
			}
			else if(Global.tool == 1 && Global.dragging == false && Global.space == false){
				checkDotSketch(this.mouseX, this.mouseY);
		...
		protected function pointOverAction(e:MouseEvent):void{
			if(Global.tool == 0 && Global.dragging == false){
				e.stopPropagation();
		...
		public function mergePath(p:Path, point:Point = null):void{ // if a point is given, attach the point to the first point of the new seglist (this is for sketch continuation)
		...
		public function joinDoubles():void{
			...
			for(var i:int=0; i<seglist.length; i++){
				for(var j:int=0; j<seglist.length; j++){
					if(i != j){
						if(seglist[i].p1 && seglist[j].p1 && seglist[i].p1 != seglist[j].p1 && joined.indexOf(seglist[i].p1) == -1 && joined.indexOf(seglist[j].p1) == -1 && Global.withinTolerance(seglist[i].p1, seglist[j].p1, 0.05)){
							seglist[i].p1 = seglist[j].p1; 
							joined.push(seglist[i].p1);
						}
						if(seglist[i].p1 && seglist[j].p2 && seglist[i].p1 != seglist[j].p2 && joined.indexOf(seglist[i].p1) == -1 && joined.indexOf(seglist[j].p2) == -1 && Global.withinTolerance(seglist[i].p1, seglist[j].p2, 0.05)){
							seglist[i].p1 = seglist[j].p2; 
							joined.push(seglist[i].p1);
						}
						if(seglist[i].p2 && seglist[j].p2 && seglist[i].p2 != seglist[j].p2 && joined.indexOf(seglist[i].p2) == -1 && joined.indexOf(seglist[j].p2) == -1 && Global.withinTolerance(seglist[i].p2, seglist[j].p2, 0.05)){
